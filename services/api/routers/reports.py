...
async def generate_report(
    ...
    reports = []
    template = getattr(request, "template", "standard") or "standard"
    for fmt in request.formats:
        report_format = ReportFormat(fmt)
        if template == "executive":
            content_bytes, content_type = _render_executive_report(
                run, findings, report_format
            )
        else:
...
<div class="footer">
    Generated by SentinelForge v2.6.0 &bull; {generated_at}<br>
...
def _build_executive_data(run: AttackRun, findings: list) -> dict:
    """Build enriched data for executive report: risk score, compliance, hardening."""
    from collections import defaultdict

    base_data = _build_report_data(run, findings)

    # Risk score: weighted severity (capped at 100)
    weights = {"critical": 25, "high": 10, "medium": 5, "low": 2, "info": 0}
    risk_score = 0
    for sev, count in base_data["severity_breakdown"].items():
        risk_score += weights.get(sev, 0) * count
    ...
    for f in base_data["findings"]:
        findings_by_severity[f["severity"]].append(f)

    # Top 3 priority actions (from highest-severity findings)
    top_actions = []
    ...
    # Compliance aggregation (4 industry frameworks, excluding Arcanum PI)
    compliance_data = []
    try:
        ...
        framework_names = {
            ...
            "owasp_ml_top10": "OWASP ML Top 10",
        ...
        for fw_id, fw_name in framework_names.items():
            categories = get_framework_categories(fw_id)
            cat_summaries = []
            for cat_id, cat in categories.items():
                count = 0
                for fd in findings_dicts:
                    tags = lookup_compliance_tags(fd.get("mitre_technique", ""))
                    for tag in tags:
                        if tag["framework"] == fw_id and tag["category_id"] == cat_id:
                            count += 1
                            break
        ...
        logger.warning(f"Compliance aggregation failed: {e}")
    ...
    try:
        ...
        hardening_recommendations = advice.get("recommendations", [])[:8]
    except Exception as e:
        logger.warning(f"Hardening advice generation skipped: {e}")
...
def _render_executive_html(data: dict) -> str:
    """Render executive report using Jinja2 template."""
    try:
        template = _jinja_env.get_template("report_executive.html.j2")
    except Exception:
        logger.warning("Executive template not found, falling back to standard report")
        return _render_html(data)

    return template.render(**data, severity_colors=_SEVERITY_COLORS)
...
def _render_executive_report(
    ...
    """Render executive report in the requested format."""
    data = _build_executive_data(run, findings)

    if fmt == ReportFormat.HTML:
        html = _render_executive_html(data)
        return html.encode("utf-8"), "text/html"
    elif fmt == ReportFormat.PDF:
        html = _render_executive_html(data)
        pdf_bytes = _html_to_pdf(html)
